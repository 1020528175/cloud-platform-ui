...
## 福利🌧
考虑许多码友对于Spring Cloud的前后端分离和微服务实战有较多的疑问，故本人将会后续展开三大课程，课程定价为`399`元，课程为直播与录播方式：
- `AG-Admin`的`框架搭建`讲解，包含Spring Cloud相关技术细节、搭建自由企业框架
- 前端Vue`交互与权限`，包含前后端分离后的权限管控、身份认证、页面按钮等相关交互和权限
- Spring Cloud`内部服务鉴权`
